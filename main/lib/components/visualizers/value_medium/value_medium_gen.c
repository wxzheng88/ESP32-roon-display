...
lv_obj_t * value_medium_create(lv_obj_t * parent, lv_subject_t * value, const char * format, const char * unit)
{
    ...
    if (!style_inited) {
        ...
        lv_style_set_text_font(&style_value, font_lg);

        lv_style_init(&style_unit);
        lv_style_set_text_font(&style_unit, font_md);

        style_inited = true;
    }

    lv_obj_t * lv_obj_0 = lv_obj_create(parent);
    lv_obj_set_name_static(lv_obj_0, "value_medium_#");
